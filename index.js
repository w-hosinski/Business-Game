...
let money = 200
const baseInputMatPrice = 1
const inputMatPrice = 1
const baseProductPrice = 2
const productPrice = 2
const machineNumber = 0
const machineCost = 14
const machinesPerManufacturingBuilding = 4
const maxMachines = 0
const incomePerMachine = productPrice-inputMatPrice
const machineRunningCost = 0.4
const machineSpeed = 1
const machineEfficiency = 1
const machineCycleTime = 1000
const rentPerManufacturingBuilding = 0.9
const manufacturingBuildingNumber  = 0
const manufacturingBuildingSetupCost = 18
const maxManufacturingBuildings = 5
const assemblerNumber = 0
const assemblerHireCost = 7
const assemblerWage = 0.54
const assemblersPerMachine = 1
const qciNumber = 0
const qciHireCost = 9
const qciWage = 0.64
const qciStrength = 0.3
const qciCapacitySquared = 3
const qciIncrementerTime = 100
const accountantNumber = 0
const accountantHireCost = 18
const accountantWage = 0.6
const maxAccountants = 0
const allEmployeesNumber = 0
const maxEmployees = 25
const maxOfficeBuildings = 5
const rentPerOfficeBuilding = 0.4
const officeBuildingNumber = 0
const officeBuildingSetupCost = 10
const accountantsPerOfficeBuilding = 4
const corpTaxRate = 0.25
const baseTaxCounter = 900
let taxCounter = baseTaxCounter
const salvageMultiple = 0.25
const reduceTaxesResearchCost = 70
const outsourcedAccountingFee = 0.04   
const termLoanInterestRates = [0.075,0.080,0.087,0.093,0.100]
const creditLimit = 100
const ccAPR = 0.185 
const ccGraceBase = 450
const advancedTrainingCostPerAssembler = 7
let liquidity = money
let creditCardInterest = 0, ccGraceCounter = 0, productsPerSecond = 0, netAnnualIncome = 0, revenuesThisQuarter = 0, costOfRevenuesThisQuarter = 0, grossProfitThisQuarter = 0, sgaThisQuarter = 0, rdThisQuarter = 0, operatingIncomeThisQuarter = 0, netInterestExpensesThisQuarter = 0, unusualItemsThisQuarter = 0, earningsBeforeTaxesThisQuarter = 0, taxExpenseApproxThisQuarter = 0, netIncomeApproxThisQuarter = 0, costOfRevenuesWriteOff = 0, oneTimeWriteOff = 0 
const tempTermLoanAmount = 0, tempTermLoanInterestRate = 0, tempTermLoanMonthlyPayment = 0, termLoan1MonthlyInterest = 0, termLoan1MonthsRemaining = 0, termLoan1MonthlyPayment = 0,  nolBalance = 0, immediateExpenseDeduction = 0,  taxExpense = 0, qciUtilisation = 0,  machineUtilization = 0, quarterCounter = 0 
...
function ticker() {
    ...
    const revenues = productsPerSecond*productPrice
    revenuesDisplay.childNodes[0].nodeValue = (revenues).toFixed(0)+" $"
    const costOfRevenues = inputMatPrice*machineEfficiency*productsPerSecond+machineRunningCost*machineNumber+assemblerNumber*(assemblerWage-outsourcedAccountingFee)+rentPerManufacturingBuilding*manufacturingBuildingNumber+costOfRevenuesWriteOff
    costOfRevenuesDisplay.childNodes[0].nodeValue = (costOfRevenues).toFixed(0)+" $"
    const grossProfit = (revenues-costOfRevenues)
    grossProfitDisplay.childNodes[0].nodeValue = (grossProfit).toFixed(0)+" $"
    const sga = qciNumber*qciWage+accountantNumber*accountantWage+rentPerOfficeBuilding*officeBuildingNumber+oneTimeWriteOff+outsourcedAccountingFee*assemblerNumber 
    sgaDisplay.childNodes[0].nodeValue = (sga).toFixed(0)+" $"
    const rd = 0
    rdDisplay.childNodes[0].nodeValue = (rd).toFixed(0)+" $"
    const operatingIncome = grossProfit-sga-rd
    operatingIncomeDisplay.childNodes[0].nodeValue = (operatingIncome).toFixed(0)+" $"
    const netInterestExpenses = termLoan1MonthlyInterest + creditCardInterest
    netInterestExpensesDisplay.childNodes[0].nodeValue = (netInterestExpenses).toFixed(0)+" $"
    const unusualItems = 0
    unusualItemsDisplay.childNodes[0].nodeValue = (unusualItems).toFixed(0)+" $"
    const earningsBeforeTaxes = operatingIncome-netInterestExpenses-unusualItems
    earningsBeforeTaxesDisplay.childNodes[0].nodeValue = (earningsBeforeTaxes).toFixed(0)+" $"
    const taxExpenseApprox = -1*(earningsBeforeTaxes*corpTaxRate)
    taxExpenseApproxDisplay.childNodes[0].nodeValue = (taxExpenseApprox).toFixed(0)+" $"
    const netIncomeApprox = earningsBeforeTaxes+taxExpenseApprox 
    ...
    const timeUntilEndOfYear = (3-quarterCounter)*(baseTaxCounter/10)+(taxCounter/10)
